  ...
  return (
    <div className="bg-background">
      ...
      <section className="section-shell py-16 md:py-24">
        <div className="rounded-2xl border bg-secondary/60 p-4 md:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="relative w-full lg:max-w-md"><Search className="absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground" /><Input type="search" placeholder="Search projects..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="h-12 rounded-xl bg-background pl-10" /></div><Tabs value={activeCategory} onValueChange={setActiveCategory}><TabsList className="grid w-full grid-cols-2 rounded-xl sm:grid-cols-4 lg:w-auto">{categories.map((category) => <TabsTrigger key={category} value={category}>{category}</TabsTrigger>)}</TabsList></Tabs></div></div>
        <div className="mt-10">{loading ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{[1,2,3,4,5,6].map(item => <Card key={item} className="overflow-hidden rounded-2xl"><div className="aspect-[4/3] animate-pulse bg-muted" /><div className="space-y-3 p-6"><div className="h-5 w-2/3 animate-pulse rounded bg-muted" /><div className="h-4 w-full animate-pulse rounded bg-muted" /></div></Card>)}</div> : filteredProjects.length > 0 ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{filteredProjects.map(project => <ProjectCard key={project.id} project={project} />)}</div> : <Card className="rounded-2xl border-dashed"><CardContent className="px-6 py-20 text-center"><FolderOpen className="mx-auto h-10 w-10 text-primary" /><h2 className="mt-5 text-2xl font-semibold">No matching projects</h2><p className="mx-auto mt-3 max-w-xl text-muted-foreground">Try another keyword or browse all categories. You can also contact us to discuss a project similar to what you have in mind.</p><Button asChild className="mt-7 rounded-xl"><Link href="/contact">Discuss your project <ArrowRight className="ml-2 h-4 w-4" /></Link></Button></CardContent></Card>}</div>
      </section>
    </div>
  );
}
