...
def _tri_request(session,start,end):
    info={
        'name':'NIFTY 50',
        'startDate':pd.Timestamp(start).strftime('%d-%b-%Y'),
        'endDate':pd.Timestamp(end).strftime('%d-%b-%Y'),
        'indexName':'NIFTY 50',
    }
    payload={'cinfo':str(info).replace('"',"'")}
    # Current NSE Indices endpoint returns the whole requested range in one response.
    # Keep the old chunked approach as a fallback for transient/legacy responses.
    r=session.post(BASE+'/BackPage/getTotalReturnIndexString',json=payload,headers=HEADERS,timeout=60)
    r.raise_for_status()
    return _tri_rows(r.json())


def tri_history(start,end):
    session=requests.Session()
    session.headers.update(HEADERS)
    # Bootstrap the same page used by the site's historical-data form; this helps
    # with Akamai/front-door behaviour in GitHub-hosted runners.
    try:
        session.get(BASE+'/reports/historical-data',timeout=10)
    except requests.RequestException:
        pass

    out=_tri_request(session,start,end)
    if len(out)<1000:
        # Fallback: retrieve the range in roughly one-year windows.
        out=[]
        cur=pd.Timestamp(start).date(); finish=pd.Timestamp(end).date()
        while cur<=finish:
            chunk_end=min(cur+timedelta(days=364),finish)
            out.extend(_tri_request(session,cur,chunk_end))
    ...
    for x in out:
        if not isinstance(x,dict):
            continue
        dt=x.get('Date') or x.get('HistoricalDate') or x.get('DATE')
        val=x.get('TotalReturnsIndex') or x.get('Total Return Index') or x.get('TRI') or x.get('TOTALRETURNINDEX')
        if val is None:
            for k,v in x.items():
                ku=str(k).upper().replace(' ','')
                if 'TOTALRETURNINDEX' in ku or ku in ('TRI','TOTALRETURN'):
                    val=v; break
        ...
        try:
            ...
            if pd.notna(d) and np.isfinite(v): rows.append((d,v))
        except (TypeError,ValueError):
            continue

    tri=pd.DataFrame(rows,columns=['Date','TRI']).drop_duplicates('Date').set_index('Date').sort_index()
    if len(tri)<1000:
        raise RuntimeError(f'Insufficient official TRI history after parsing: {len(tri)} rows')
    return tri
