...
def run(data,start,end,p):
    ...
    for dt in idx:
        ...
        for s,d in data.items():
            if dt not in d.index:
                total += shares[s]*float(d.Close.loc[:dt].iloc[-1]) if shares[s] else cash[s]
                continue
            ...
            if shares[s]>0:
                ...
                if reason:
                    ...
                    proceeds=shares[s]*ex*(1-p['cost'])
                    tr=proceeds/invested[s]-1 if invested[s] else 0
                    cash[s]=proceeds
                    trades.append({'stock':s,'entry':str(entry[s].date()),'exit':str(dt.date()),'return':tr,'reason':reason,'invested':invested[s],'proceeds':proceeds})
    ...
    cagr=e.iloc[-1]**(1/years)-1; dd=(e/e.cummax()-1).min(); sh=dr.mean()/dr.std()*np.sqrt(252) if dr.std() else 0
...
def main():
    ...
    annual={str(k.year):float(v) for k,v in full['equity'].resample('YE').last().pct_change().dropna().items()}
    stress={}
    for tc in [.001,.002,.005,.01]:
        p=dict(BASE_P); p['cost']=tc; p['slip']=tc; r=run(data,common,end,p)
        stress[f'{tc*100:.1f}%']={k:r[k] for k in ['cagr','max_drawdown','sharpe','total_return']}
    oos_start=pd.Timestamp('2020-01-01'); oos=run(data,oos_start,end,BASE_P); pre=run(data,common,pd.Timestamp('2019-12-31'),BASE_P)
