...
def tri_history(start,end):
    ...
    for x in out:
        ...
        if val is None:
            for k,v in x.items():
                if k in ('HistoricalDate','DATE','Date'): continue
                try:
...
def run(d,max_cut=0.10):
    c=d.Close.astype(float); tri=d.TRI.astype(float); r_tri=tri.pct_change().fillna(0.0)
    ma50=c.rolling(50).mean(); ma200=c.rolling(200).mean(); mom63=c.pct_change(63); mom252=c.pct_change(252)
    equity=1.0; peak=1.0; exposure=1.0; eq=[]; trades=0
    for i in range(1,len(d)):
        p=i-1; vals=[ma50.iloc[p],ma200.iloc[p],mom63.iloc[p],mom252.iloc[p],d.VIX.iloc[p]]
        if not all(np.isfinite(x) for x in vals):
            equity*=1.0+r_tri.iloc[i]; peak=max(peak,equity); eq.append((d.index[i],equity)); continue
        m50,m200,m63,m252,vix=vals; dd=equity/peak-1.0; cut=0.0
        ...
        if weekly and abs(target-exposure)>=0.025:
            turnover=abs(target-exposure); equity*=max(0.0,1.0-turnover*(COST+SLIPPAGE)); exposure=target; trades+=1
        equity*=1.0+exposure*r_tri.iloc[i]; peak=max(peak,equity); eq.append((d.index[i],equity))
...
def metrics_window(eq,a,b):
    s=eq.loc[a:b]
    if len(s)<2: return {}
    s=s/s.iloc[0]; rr=s.pct_change().fillna(0); years=(s.index[-1]-s.index[0]).days/365.25
...
def main():
    ...
    out={'strategy':'V9 corrected NIFTY core + 10% maximum defensive overlay','data_start':str(d.index[0].date()),'data_end':str(d.index[-1].date()),'benchmark':'Official NIFTY 50 Total Return Index','buy_hold_tri_cagr':bench,'full':m,'oos':oos,'execution':'previous close signal, weekly next-open rebalance; TRI returns drive portfolio equity','cost':COST,'slippage':SLIPPAGE,'research_note':'Corrected accounting: no mixing of price-index levels with TRI levels. Signals use NIFTY price index; portfolio returns use official TRI daily returns. OOS windows are rebased independently. No OOS parameter selection.'}
