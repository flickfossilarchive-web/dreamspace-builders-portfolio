...
def run(d):
    ...
    for i in range(1,len(d)):
        ...
        if regime and momentum:
            raw=min(MAX_EXPOSURE,TARGET_VOL/max(float(vol.iloc[p]),0.05)); target=max(0.50,min(1.0,raw))
        elif close>float(sma200.iloc[p]) and float(mom126.iloc[p])>0: target=0.35
        else: target=0.0
        if new_month and target!=last_target:
            total=cash+units*op; desired_value=total*target; desired_units=desired_value/(op*(1+SLIPPAGE)); delta=desired_units-units
            if delta>1e-12:
                cash-=delta*op*(1+SLIPPAGE); units+=delta; trades.append({'date':str(d.index[i].date()),'side':'BUY','exposure':target})
            elif delta< -1e-12:
                qty=-delta; cash+=qty*op*(1-SLIPPAGE)*(1-COST); units-=qty; trades.append({'date':str(d.index[i].date()),'side':'SELL','exposure':target})
            last_target=target
        rows.append((d.index[i],cash+units*float(c.iloc[i])))
    eq=pd.Series(dict(rows)).sort_index(); ret=eq.pct_change().fillna(0); years=(eq.index[-1]-eq.index[0]).days/365.25
    cagr=eq.iloc[-1]**(1/years)-1; dd=(eq/eq.cummax()-1).min(); sharpe=ret.mean()/ret.std()*np.sqrt(252)
...
def main():
    d=load_data(); r=run(d); bh=buy_hold(d); windows=[('2007-09-17','2009-12-31'),('2010-01-01','2019-12-31'),('2020-01-01',str(d.index[-1].date()))]
    oos=[]
    for a,b in windows:
