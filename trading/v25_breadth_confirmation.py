...
def run(d, broad, cost=COST, slippage=SLIPPAGE, trend=TREND, breadth_trend=BREADTH_TREND, relative_mom=RELATIVE_MOM):
    n = d.Close.astype(float); tri = d.TRI.astype(float); b = broad.broad.reindex(d.index).ffill()
    ma_n = n.rolling(trend).mean(); ma_b = b.rolling(breadth_trend).mean()
    rel = b.pct_change(relative_mom) - n.pct_change(relative_mom); nr = tri.pct_change().fillna(0)
    w = 1.0; eq = 1.0; vals = []; trades = 0; turnover = 0.0; exposures = []
    for i in range(1, len(d)):
        ...
        if weekly and all(np.isfinite(x) for x in (ma_n.iloc[p], ma_b.iloc[p], rel.iloc[p])):
            n_up = n.iloc[p] > ma_n.iloc[p]; b_up = b.iloc[p] > ma_b.iloc[p]; rel_up = rel.iloc[p] > 0
            target = 1.0 if (n_up and b_up and rel_up) else (0.70 if ((not n_up) and (not b_up) and (not rel_up)) else 0.90)
            if abs(target - w) >= 0.05:
                t = abs(target - w); turnover += t; eq *= max(0.0, 1.0 - t * (cost + slippage)); w = target; trades += 1
        eq *= 1.0 + w * nr.iloc[i]; vals.append((d.index[i], eq)); exposures.append(w)
...
def main():
    d = data(); broad = load_broad(str(d.index[0].date()), str((d.index[-1] + pd.Timedelta(days=1)).date())); d = d.join(broad, how='inner').dropna()
    tri = d.TRI; yrs = (tri.index[-1] - tri.index[0]).days / 365.25; bench = float((tri.iloc[-1] / tri.iloc[0]) ** (1 / yrs) - 1)
    base_s, base = run(d, broad)
    costs = {str(int(c * 10000)): run(d, broad, cost=c, slippage=c)[1] for c in [0, .0005, .001, .0015, .002, .003, .004, .005]}
    perturbations = {}
    for name, tr, bt, rm in [('trend150',150,200,126),('trend250',250,200,126),('breadth150',200,150,126),('breadth250',200,250,126),('rel90',200,200,90),('rel180',200,200,180),('rel60',150,150,60),('all_long',9999,9999,126)]:
        if name == 'all_long':
            rr=tri.pct_change().dropna(); perturbations[name]={'cagr':bench,'max_drawdown':float((tri/tri.cummax()-1).min()),'sharpe':float(rr.mean()/rr.std()*np.sqrt(252)),'trades':0,'turnover':0.0,'avg_nifty_weight':1.0}
        else: perturbations[name] = run(d, broad, trend=tr, breadth_trend=bt, relative_mom=rm)[1]
    ...
    out={'strategy':'V25 NIFTY trend with NIFTY 500 breadth/relative-strength confirmation','benchmark_cagr':bench,'base':base,'cost_sensitivity':costs,'parameter_perturbations':perturbations,'walk_forward':wf,'rules':{'risk_on':'NIFTY > MA200 AND NIFTY500 > MA200 AND NIFTY500 126d relative momentum vs NIFTY > 0 => 100%','risk_off':'all three conditions negative => 70%','neutral':'otherwise => 90%','rebalance':'weekly thresholded','signals':'previous session only','leverage':'none','breadth_proxy':'NIFTY 500 index, not current constituents'},'gate':{'cost_bps_primary':20,'required_base_cagr_gt_benchmark':True,'required_20bps_cagr_gt_benchmark':True,'required_all_perturbations_positive':True,'required_all_walk_forward_positive':True,'required_sharpe_ge':0.80,'required_max_dd_better_than':-0.30}}
    json.dump(out, open('v25_results.json','w'), indent=2); print(json.dumps(out,indent=2))
