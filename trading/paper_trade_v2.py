...
def signal(px,mem,asof,top_n=20):
    ...
    current=px.loc[:asof].ffill(limit=2).iloc[-1]
    eligible=[]
    for s in px.columns:
    ...
    return picks,{s:1/top_n for s in picks},current
...
def main():
    ...
    px=fetch_prices(symbols,'2014-01-01',end)
    ...
    picks,w,current=signal(px,mem,asof)
    bm=yf.download('^NSEI',start='2014-01-01',end=end,auto_adjust=True,progress=False)['Close']
    if isinstance(bm,pd.DataFrame): bm=bm.iloc[:,0]
    scale,bull,vol=risk_scale(px,picks,asof,bm)
    rows=[{'symbol':s,'target_weight':w[s]*scale,'reference_price':float(current[s])} for s in sorted(picks)]
    portfolio=pd.DataFrame(rows); cash=float(1-scale)
